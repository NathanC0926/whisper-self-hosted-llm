    ...
    try:
        ...
        if response.status_code == 200:
            try:
                ...
                st.error("Failed to decode the server response. The response was not valid JSON.")
        else:
            # Log the error
            logging.error(f'Failed to get a valid response: Status Code {response.status_code}, Response: {response.text}')
